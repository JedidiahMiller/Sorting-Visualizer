...
export default class BarManager {
  ...
  swapVisualBars(index1, index2, onFinish) {
    ...
    const onFinishFunction = () => {
      obj.value ? onFinish() : obj.value = true;
    }

    // Position update
    this.#visualState[index1].setPosition(this.#getPositionFromIndex(index1), {onFinish: onFinishFunction});
    this.#visualState[index2].setPosition(this.#getPositionFromIndex(index2), {onFinish: onFinishFunction});
